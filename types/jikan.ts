...
export interface AnimeQuery {
  ...
  genres?: string
  /** `tv` | `movie` | `ova` | `special` | `ona` | `music`, per Jikan. */
  type?: string
  /** `airing` | `complete` | `upcoming`, per Jikan's `status` filter. */
  status?: string
  /** Default ordering for the discovery feed. */
  order_by?: string
  sort?: 'asc' | 'desc'
  /** Minimum score (0–10) for the listing filter. */
  min_score?: number
  sfw?: boolean
  limit?: number
}
...
/**
 * A single recommendation entry from `/anime/{id}/recommendations`.
 * `entry` is a slim Anime-like card (id, title, images), `votes` ranks it.
 */
